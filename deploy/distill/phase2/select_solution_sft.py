#!/usr/bin/env python3
"""Select verified candidates for solution SFT and identify weak groups."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input", required=True, help="Verified candidate JSONL")
    ...
    parser.add_argument("--weak-ids-output", default=None, help="Optional newline-delimited failed IDs from weak groups")
    parser.add_argument("--max-per-task", type=int, default=0, help="Maximum passing samples per source task; 0 keeps all")
    parser.add_argument("--min-passing-per-task", type=int, default=10, help="Groups below this count are marked weak")
...
def export_solution_row(sample: dict[str, Any], include_tests: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a precise coding assistant. Return production-quality code and focused tests when requested.",
            },
            {
                "role": "user",
            ...
            {
                "role": "assistant",
                "content": assistant_content(sample, include_tests),
            },
        ],
        "verification": {
...
def main() -> None:
    ...
    weak_ids_path = Path(args.weak_ids_output) if args.weak_ids_output else output_path.with_suffix(".weak_failed_ids.txt")
    ...
    for group_id, group_samples in sorted(groups.items()):
        ...
        weak = len(passed) < args.min_passing_per_task
        if weak:
            weak_failed_ids.extend(sample.get("id", "") for sample in failed)
    ...
    weak_ids_path.write_text("\n".join(id_ for id_ in weak_failed_ids if id_) + ("\n" if weak_failed_ids else ""))

    manifest = {
        "input": str(input_path),
        "output": str(output_path),
        "weak_ids_output": str(weak_ids_path),
        "total_samples": len(samples),
        "groups": len(groups),
        "selected": len(selected),
        "max_per_task": args.max_per_task,
        "min_passing_per_task": args.min_passing_per_task,
        "weak_groups": [group_id for group_id, row in by_task.items() if row["weak"]],
        "weak_failed_ids": len(weak_failed_ids),
        "by_task": by_task,
    ...
    print(json.dumps({
        ...
        "weak_groups": manifest["weak_groups"],
        "weak_failed_ids": manifest["weak_failed_ids"],
    }, indent=2))
