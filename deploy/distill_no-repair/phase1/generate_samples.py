...
TEST_SYSTEM_PROMPT = (
    ...
    "Use Vitest-compatible tests with minimal mocks. Assert public behavior only. "
    "Import only from the solution module (./solution) and standard test APIs. "
    "Do not use real timers, network, or undeclared npm packages."
)
...
def main() -> None:
    ...
    prompts_path = Path(env_value(args, "prompts", "PROMPTS", "deploy/distill_no-repair/phase1/prompts/pilot_tasks.jsonl"))
    output_path = Path(env_value(args, "output", "OUTPUT", "deploy/distill_no-repair/phase1/outputs/pilot.jsonl"))
